/*
Modified MOUSE JACK code to find the Logitech r400 presenter
Button presses on presenter found in the payload


TO DO:
-Use vibration motor with button press
...
void print_payload_details()
{
  ...
  if (payload[3] == 0x4B)
  {
    Serial.println("LEFT ARROW BUTTON PRESSED <<");
    green_button_light();
    
  }
  else if( payload[3] == 0x4E)
  {
    Serial.println("RIGHT ARROW BUTTON PRESSES >>");
    red_button_light();
  }
  Serial.println("");
  return;
...
void sweep_channel()
{
  // this zeros in on the channel where the presenter is found
  if (sweep)
  {
    //flash blue two times to say read
        ...
        for (offset = 0; offset < 2; offset++) {
          ...
          if (payload_length <= (PAY_SIZE-9))
          {
            ...
            if (crc == crc_given) {
              ...
              sweep = false;
              // set the channel to this found channel
...
void fingerprint()
{
  ...
  if (payload[0] == 0 || payload[0] == 7) {
    if (payload_size == 10 && (payload[1] == 0xC1 || payload[1] == 0x4F))
      payload_type = LOGITECH;
    if (payload_size == 22 ) //&& payload[1] == 0xD3)
      payload_type = LOGITECH;
    if (payload_size == 5 && payload[1] == 0x40)
      payload_type = LOGITECH;
    if (payload_type == LOGITECH) Serial.println("found Logitech presenter!!");
  }
  return;
}
